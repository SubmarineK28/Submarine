...
def assembly_layer_construct(id=100,
    ...
    H_fuel_assembly = 9.52
    bord_inner = get_plane(H=H_out, boundary="transmission", )
    bord_outer = get_plane(H=H_fuel_assembly, boundary="transmission")
    ...
    # 2 мм = 0.2 см (толщина по нормали к грани)
    water_layer_cm = 0.125

    # H в вашем коде — "под ключ", поэтому добавляем 2*толщину
    H_with_extra_water = subassembly_duct + 2.0 * water_layer_cm

    # Новая внешняя граница (с учетом доп. водяного слоя)
    bord_outer_water = get_plane(H=H_with_extra_water, boundary=boundary)

    # Слой воды между старой внешней границей и новой
    outer_water_cells = set_chanel_3d(
        bord_outer=bord_outer_water,
        bord_inner=bord_duct_outer,   # старая внешняя граница сборки
        p1=p1, p2=p2, p3=p3,
        z_up=zup, z_down=zdn,
        material=coolant,
        name="extra outer water 2mm"
    )
    ...
    for one_cell in outer_coolant_cells.values():
        fuel_assembly_universe.add_cell(one_cell)
    for one_cell in outer_water_cells.values():
        fuel_assembly_universe.add_cell(one_cell)    

    root_cell = openmc.Cell(name=cell_name, fill=fuel_assembly_universe)
    root_cell.region = (
        -bord_outer_water[1] & -bord_outer_water[2] & -bord_outer_water[3] &
        -bord_outer_water[4] & -bord_outer_water[5] & -bord_outer_water[6] &
        -zup & +zdn 
    )
